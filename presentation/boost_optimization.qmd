...
1.  Driver model
    -   Identifies drivers in synthetic trip data so we can optimize their behavior
...
-   Needs to solve the cold start problem for new drivers with no price sensitivity data. Perhaps a collaborative filtering-type approach.

-   Additional analysis of individual driver price sensitivity is warranted. A linear model for each driver might be more persuasive, but would require more data. The nonparametric model does not always show an increase in probability when there is an increase in boost offer size.
