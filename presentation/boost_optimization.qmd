...
3.  How can we both minimize boost claim time and average cost of rides?\
...
-   Understand the available trip data
-   Generate synthetic driver data for demonstration purposes
-   Predict rides that are likely to go unclaimed
-   Model the price sensitivity of each driver (in each area for each trip)
-   Predict the optimal (minimal) boost amount for each driver that will move that trip over the claim probability threshold
-   Analyze optimized boost amounts against the original claimed amounts
...
writeLines("How do trips breakdown by geographic area?")
...
-   Additional analysis of individual driver price sensitivity is warranted. A linear model for each driver might be more persuasive, but would require much more data. The nonparameric model does not always show an increase in probability when there is an increase in boost offer size.
